...
$manifest = Join-Path $rootPath "handlers.manifest.json"
$warn = {
...
if (Test-Path $manifest) {
    try {
        ...
        & $warn "handlers.manifest.json is invalid. Falling back to file scan."
    }
}
if (-not $list -or $list.Count -eq 0) {
    $list = Get-ChildItem -Path $rootPath -Filter "handlers-*.ps1" |
        Where-Object { $_.Name -notin @("handlers.ps1","handlers-loader.ps1") } |
        Sort-Object Name |
        Select-Object -ExpandProperty Name
}
if ($SyncManifest) {
    $current = Get-ChildItem -Path $rootPath -Filter "handlers-*.ps1" |
        Where-Object { $_.Name -notin @("handlers.ps1","handlers-loader.ps1") } |
        Sort-Object Name |
        Select-Object -ExpandProperty Name
    ...
    if ($missing.Count -gt 0 -or $extra.Count -gt 0) {
        ...
        try {
            ...
            & $warn "Failed to sync handlers.manifest.json."
        }
        $list = $newList
    }
}
...
if ($verbose) {
    Write-Host ("[handler] loaded " + $list.Count + " files")
}
